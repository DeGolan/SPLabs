...
void relocationTables(elf *e)
{
    ...
    char *sh_strtab_p = e->addr + sh_strtab->sh_offset;
        for (int i = 0; i < e->ehdr->e_shnum; i++)
    {
        if (strcmp(sh_strtab_p + e->shdr[i].sh_name, ".dynsym") == 0)
        {
             printf("found dunsym table at %s\n",e->addr + e->shdr[i].sh_offset);
            dyntab = (Elf32_Sym *)(e->addr + e->shdr[i].sh_offset);
        }
        if (strcmp(sh_strtab_p + e->shdr[i].sh_name, ".dynstr") == 0)
        {
            printf("found shit\n");
            dynstrtab = (char *)(e->addr + e->shdr[i].sh_offset);
        }
    }
    for (int i = 0; i < e->ehdr->e_shnum; i++)
    {
        if (strcmp(getSectionType(e->shdr[i].sh_type), "SHT_REL") == 0)
        {
            num_of_relations=(e->shdr[i].sh_size/8);
            printf("Relocation section '%s' at offset 0x%x contains %d entries:\n",sh_strtab_p + e->shdr[i].sh_name, e->shdr[i].sh_offset,num_of_relations);
            reltab = (Elf32_Rel *)(e->addr + e->shdr[i].sh_offset);
            printf("OFFSET\tINFO\tTYPE\tVALUE\t\n");
            for (int j = 0; j< num_of_relations; j++)
            {
                int index=ELF32_R_SYM(reltab[j].r_info);
                Elf32_Sym* curr = dyntab+index;
                
                int type = ELF32_R_TYPE(reltab[j].r_info);
                //printf("%x\t%x\t%d\n",reltab[j].r_offset,reltab[j].r_info,type);

                // printf("%x\t%x\t%d\t%x\t\n",reltab[j].r_offset,reltab[j].r_info,type,(dyntab[index]).st_value);
                  printf("%x\t%x\t%d\t%x\t%s\t\n",reltab[i].r_offset,reltab[i].r_info,type,curr->st_value,dynstrtab+curr->st_name);
            }
        }
    }
}
