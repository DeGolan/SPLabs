...
void detect_virus(char *buffer, unsigned int size, link *virus_list, FILE *output)
{
    ...
    while (temp != NULL)
    {
        while (i < size - temp->vir->SigSize)
        {
            if (memcmp(buffer + i, temp->vir->sig, temp->vir->SigSize) == 0)
            {
                printf("Byte start pos:%d", i);
                printf("Virus name: %s", temp->vir->virusName);
                printf("Virus sig size: %d", temp->vir->SigSize);
            }
            i++;
        }
        temp = temp->nextVirus;
...
link *detectViruses(link *viruses)
{
    ...
    fclose(input);
    detect_virus(buffer, fsize, viruses, stdout);
    return viruses;
}
