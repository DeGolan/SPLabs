...
virus* readVirus(FILE* input){
    ...
    fread(newVirus,1,18,input); //reading SigSize and VirusName to the new virus
    newVirus->sig=(unsigned char*)calloc (sizeof(unsigned char*)*(newVirus->SigSize),1);
    fread(newVirus->sig,1,newVirus->SigSize,input);//reading sig to the new virus
    return newVirus;
}
...
link* list_append(link* virus_list, virus* data){
    ...
    if(virus_list==NULL){//for empty list
...
void list_free(link *virus_list){
    ...
    while (tmp!=NULL){
        link* tmp2=tmp;
        tmp=tmp->nextVirus;
        free(tmp2->vir->sig); //free the virus signature
        free(tmp2->vir); //free the virus itself
        free(tmp2); //free the link
    } 
}
